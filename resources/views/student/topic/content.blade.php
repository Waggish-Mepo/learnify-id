@extends('layouts.app')

@section('content')
<div class="block-header px-4">
    <div class="d-flex flex-column m-auto pt-md-5 pt-4">
        <h3 class="text-capitalize text-white text-center">materi bentuk-bentuk aljabar | minggu 1</h3>
        <div class="h-auto p-4 mt-md-3 mt-2 w-100 bg-white shadow-sm rounded">
            <div class="d-flex justify-content-between mb-3">
                <a href="#" class="color-blue-2 d-flex align-items-center js-sweetalert" data-type="confirm-back" data-toggle="tooltip" title="Kembali">
                    <i class="fa fa-sign-out"></i>
                    <span class="ml-1">Keluar</span>
                </a>
                <div></div>
            </div>
            <p class="text-justify color-black">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Minima reiciendis ut atque in ex eos voluptatibus. Aspernatur reiciendis modi minima in perferendis nihil, animi accusantium enim? Porro cum saepe dolorem nulla quidem! Fugit cupiditate quae molestiae fuga, dolorem dolore distinctio minus esse delectus, nam porro quo tempore sunt maxime, temporibus dolor illo tenetur exercitationem libero. Consectetur a cum omnis ea ut quos laboriosam eius in debitis suscipit quo accusamus rem unde, iste dolor molestiae libero maxime quia porro amet iusto adipisci. Eaque possimus enim repudiandae reiciendis facilis inventore laborum numquam! Natus, sint fugit. Cumque ex minus, soluta sed voluptate doloremque.
                <br>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse molestias libero saepe consequuntur distinctio laborum accusantium temporibus provident sapiente, autem odio molestiae sint expedita placeat, repellat nobis at sit similique? Eveniet consequuntur, temporibus quia praesentium vitae eligendi, sit maxime nihil repellat officiis, quam debitis eaque qui expedita sed? Consequuntur voluptatibus, nesciunt necessitatibus minus doloribus omnis veritatis quia numquam voluptatum, consequatur sequi voluptas reprehenderit dolorum illo ex consectetur? Facere non ea quis commodi eos iure consectetur quasi debitis iste eaque est, eum modi porro voluptate sed numquam repellat temporibus explicabo, ad quaerat cum. Cupiditate ipsa illum obcaecati debitis fugit quidem porro, neque magni aliquid, ullam molestias, doloribus eos quod a placeat repellendus.
            </p>
            <button type="button" class="btn bg-blue-2 text-white mt-3 float-right text-capitalize">tandai telah dibaca!</button>
        </div>
    </div>
</div>
@endsection

@section('script')
<script src="{{asset('assets/js/pages/custom.js')}}"></script>
@endsection